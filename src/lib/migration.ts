...
const create_drn_for_review_excel = async (user: EdmsUser, tm: WorkProc) => {
    ...
    newProc.due_date = tm.due_date;
    newProc.create_by = user.username;
    newProc.create_tm = tm.create_tm;
    ...
    requesterAssign.create_tm = tm.create_tm;
    requesterAssign.due_to_date = tm.due_date;
...
const DOCU_RANGE_REGEX = /(\d+)~(\d+)/gi;
const DOCU_LAST_NUBMER_REGEX = /(\d+)(?!.*\d)[^-]/gi;
const DOCU_SUBJECT_ONLY_CHAR = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+┼<>@\#$%&\ '\"\\(\=]/gi;
var make_user: EdmsUser;
const migration_tr = async () => {
    //#region 초기설정
    const database = new Database("hdc_db");
    await database.getConnection();
    // await getConnection().transaction(async tr=>{
    //     let work_proc = await tr.getRepository(WorkProc).find({ is_use : 1, wp_type : "TM" });

    //     for(var wp of work_proc){
    //         if(wp.due_date){
    //             let due_date = addWorkingDay(wp.due_date, 10).toDate();
    //             await tr.createQueryBuilder().update(WorkProc).set({ due_date : due_date }).where(`wp_idx = ${wp.wp_idx}`);
    //             await tr.createQueryBuilder().update(WorkAssign).set({ due_to_date : due_date }).where(`wp_idx = ${wp.wp_idx}`);
    //         }
    //     }
    // })
    // let work_assign = await getRepository(WorkAssign).find({  })
    // return;
    //#endregion
    //#region 프로젝트 타입 등 DB에서 가져오기
    const tr_path: string = "C:\\Users\\Administrator\\Documents\\TR_BACKUP\\DocuResult_20220307\\T-TH(H)S-TEP (송전)"; //await ReadUserInputText();
    const type_index: number = 2; //await ReadUserInputSelect(Object.keys(migration_type));
    const skipTrCount = 0;
    const tr_list_excel_path =
        "C:\\Users\\Administrator\\Documents\\TR_BACKUP\\DocuResult_20220307\\TEP_TR Issue Log_220514.xlsx";
    let trIssueLog = GetExcelFileData({ mimetype: "xls", path: tr_list_excel_path });
    let trListData: {
        planDt: Date;
        issuedDt: Date;
        trNo: string;
    ...
    let trListPage = 0;
    for (; trListPage < trIssueLog.length; trListPage++) {
        let trData = trIssueLog[trListPage].data;
        let trDataLength = trData.length;
        let trDataIdx = 0;
        let trNo = -1;
        let docNo = -1;
        let rev = -1;
        ...
        for (; trDataIdx < trDataLength; trDataIdx++) {
            let row = trData[trDataIdx];
            if (trNo != -1 && docNo != -1 && desc != -1) {
                if (row[docNo] && row[docNo].indexOf("~") != -1) {
                    let regexp = new RegExp(DOCU_RANGE_REGEX);
                    let reg = regexp.exec(row[docNo]);
                    if (reg != null && reg[0] && reg[1] && reg[2]) {
                        let startIdx = parseInt(reg[1]);
                        let endIdx = parseInt(reg[2]);
                        let idx = 1; // description 뒤에 숫자를 붙여준다.
                        for (; startIdx <= endIdx; startIdx++, idx++) {
                            trListData.push({
                                planDt: ExcelDateToJSDate(row[planDt]),
                                issuedDt: ExcelDateToJSDate(row[issuedDt]),
                                trNo: row[trNo],
                                docNo: row[docNo] ? row[docNo].replace(reg[0], startIdx) : "",
                                rev: row[rev],
                                desc: row[desc] ? row[desc].replace(DOCU_SUBJECT_ONLY_CHAR, "") + idx : "",
                                checked: [],
                            });
                        }
                    }
                } else {
                    // let lastNumber = DOCU_LAST_NUBMER_REGEX.exec(row[docNo]);
                    // if(lastNumber && lastNumber.length > 2){}
                    trListData.push({
                        planDt: ExcelDateToJSDate(row[planDt]),
                        issuedDt: ExcelDateToJSDate(row[issuedDt]),
                        trNo: row[trNo],
                        docNo: row[docNo],
                        rev: row[rev],
                        desc: row[desc] ? row[desc].replace(DOCU_SUBJECT_ONLY_CHAR, "") : "",
                        checked: [],
                    });
                }
            } else if (row) {
                row.map((raw, idx) => {
                    if (raw && typeof raw == "string") {
                        if (raw.indexOf("TR No") != -1) trNo = idx;
                        else if (raw.indexOf("Document No") != -1) docNo = idx;
                        else if (raw.indexOf("Rev") != -1) rev = idx;
                        else if (raw.indexOf("Description") != -1) desc = idx;
                        else if (raw.indexOf("Plan Date") != -1) planDt = idx;
                        else if (raw.indexOf("Issued Date") != -1) issuedDt = idx;
                    }
                });
            }
        }
    ...
    let project = await getRepository(EdmsProjectType).findOne({ project_code: Like(`%${p_type}%`) });
    ...
    if (project) {
        console.log(`${project.project_name} TR Exist ? ${fs.existsSync(tr_path)}`);
        if (fs.existsSync(tr_path)) {
            await unzipInFolder(tr_path, skipTrCount); // unzip all zipfile in folder
            ...
            for (var result of tr_results) {
                ...
                try {
                    ...
                    let exist_work_tm = await getRepository(WorkTm).findOne({ tm_code: code });
                    if (exist_work_tm != undefined) {
                        console.log(folderName + " TR is pass why tr exist");
                        continue;
                    }
                    let trDocuments = trListData.filter(raw => raw.trNo == code);
                    let real_file_list = [];
                    ...
                    result.items.map(raw => {
                        ...
                        // 2. 파일명에 도큐먼트 코드와 issue log 비교가 안되면, 파일명으로 참조
                        if (isDocument == -1) {
                            isDocument = trDocuments.findIndex(docu => {
                                let desc_search = hangul.rangeSearch(itemPath, docu.desc);
                                console.log("????????????????????????????", itemPath, docu.desc, desc_search);
                                let isFindHangul = desc_search.length > 0 && desc_search[0].length > 0 ? true : false;
                                return (
                                    itemPath.indexOf("zip") == -1 && // 파일명에 zip 들어가면 압축파일 인거로 간주하고 제외
                                    (itemPath.indexOf(docu.desc) != -1 ||
                                        itemPath.indexOf(docu.docNo) != -1 ||
                                        isFindHangul)
                                );
                            });
                        }
                    ...
                    if (unreal.length > 0) {
                        let unreal_documents = await getRepository(EdmsDocument).find({
                            docu_code: In(unreal.map(raw => raw.docNo)),
                        });
                    ...
                    // let real_file_list = result.items.filter(raw => get_document_number(raw).docu_code != null);
                    // let comment_file = result.items.find(raw => raw.indexOf("xls") != -1);
                    // let attach_file_list = result.items.filter(
                    //     raw => !real_file_list.find(real_file => real_file == raw) && comment_file != raw
                    // );
                    console.log(
                    ...
                    let allFileDatas = real_file_list
                        .map((_real_file, idx) => {
                            let { docu_code, revision, revisionOrigin, revisionType } = get_document_number(_real_file);
                            let findTrDocu = trDocuments.find(raw => raw.checked.indexOf(idx) != -1);
                            console.log(findTrDocu, docu_code);
                            ...
                            if (document == undefined) {
                                document = docu_list.find(
                                    raw =>
                                        findTrDocu &&
                                        (findTrDocu.docNo === raw.docu_code ||
                                            raw.docu_subject
                                                .replace(DOCU_SUBJECT_ONLY_CHAR, "")
                                                .indexOf(findTrDocu.desc) != -1)
                                );
                            }
                            // 제목으로도 없으면 에러
                            if (document == undefined) {
                    ...
                    const issuedDate =
                        trDocuments && trDocuments.length > 0 && trDocuments[0].issuedDt
                            ? trDocuments[0].issuedDt
                            : new Date();
                            ...
                            await getConnection().transaction(async tr => {
                                for (var i = 0; i < doc_ids.length; i++) {
                                    ...
                                    if (edms_file) {
                                        ...
                                        // let find_stages =
                                        //     edms_stages.length > 0
                                        //         ? edms_stages.filter(
                                        //               stage =>
                                        //                   stage.docu_no == docu_no &&
                                        //                   stage.stage_code == now_stage.stage_name &&
                                        //                   stage.stage_type.toLocaleLowerCase() ==
                                        //                       stage_type.toLocaleLowerCase()
                                        //           )
                                        //         : [];
                                        // let find_stage: EdmsStage;
                                        // if (find_stages.length > 0) {
                                        //     find_stage = find_stages.sort((a, b) => b.revision - a.revision)[0];
                                        // } else {
                                        //     console.log("STAGE 가 없음!!! : " + docu_no);
                                        // }
...
//#region edms_files revision 강제수정코드!!
// await getConnection().transaction(async tr => {
//     let files = await tr.getRepository(EdmsFiles).find();
//     for (var file of files) {
//         const { revisionOrigin } = get_document_number(file.original_file_name);
//         if(revisionOrigin)
//         await tr
//             .createQueryBuilder()
//             .update(EdmsFiles)
//             .set({ revision: revisionOrigin, modify_tm: new Date() })
//             .where("file_no = :id", { id: file.file_no })
//             .execute();
