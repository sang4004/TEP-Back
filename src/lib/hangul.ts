...
const disassemble = function (string ?: any, grouped ?: any) {
    /* 입력값이 NULL일 경우 에러 발생 */
    if (string === null) {
    ...
    for (var i = 0; i < length; i++) {
        ...
        if (_isHangul(code)) {
            ...
            cho = parseInt(((code - jong) / 28 / 21).toFixed(0));
        ...
        for (var step = 1; ; step++) {
            if (step === 1) {
                ...
                hangul = String.fromCharCode(
                    (CHO_HASH[cho] * 21 + JUNG_HASH[jung1]) * 28 + JONG_HASH[jong1] + HANGUL_OFFSET
                );
            } else if (step === 4) {
